...
import { BadRequestException, HttpStatus, InternalServerErrorException, NotFoundException, Query } from '@nestjs/common';
...
describe('BooksController', () => {
    ...
    describe('getBooks / GET', () => {
        it('successfully returns books', async () => {
            ...
            jest.spyOn(bookService, 'getBooks').mockImplementationOnce(async ({}) => result);
        ...
        it('throws a BadRequestException when multiple query params', async () => { 
            jest.spyOn(bookService, 'getBooks').mockImplementationOnce(async () => {
    ...
    describe('getBook /:id GET', () => {
        ...
        it('sucessfully returns book', async () => {
            jest.spyOn(bookService, 'getBook').mockImplementationOnce(async (id:ObjectId) => result)
        ...
        it('throws a not found', async () => {
            jest.spyOn(bookService, 'getBook').mockImplementationOnce(async (id:ObjectId) => null)
            await expect(async () => {
                await controller.getBook(id, mockRes)
    ...
    describe('searchBooks /search GET', () => {
        ...
        it('returns the searched results', async () => {
            jest.spyOn(bookService, 'search').mockImplementationOnce(async (q:string) => result)
    ...
    describe('getData /find/:data GET', () => {
        it('throws bad request when missing id', async () => {
            jest.spyOn(bookService, 'getBook').mockImplementationOnce(async id => {
                return {} as any
            })
            await expect(async () => {
        ...
        it('returns a value', async () => {
            ...
            const book = await controller.getData('title', 'randomid' as any) as any;
            // Assertions
        ...
        it('throws a not found', async () => {
            ...
            await expect(async () => await controller.getData('title', 'randomid' as any))
                .rejects.toThrow(NotFoundException)
        });
        ...
        it('throws a bad request', async () => {
            ...
            expect(async () => {
                await controller.getData('randomshit' as any, 'randomid' as any) as any;
            }).rejects.toThrow(BadRequestException)
        });
    })
    ...
    describe('addBook / POST', () => {
        it('returns a result', async () => {
            const result = {} as any
            jest.spyOn(bookService, 'add').mockImplementationOnce(b => result)
            expect(await controller.addBook({} as any)).toBeDefined()
        })
        it ('throws a 500', () => {
            jest.spyOn(bookService, 'add').mockImplementationOnce(b => null as any)
            expect(async () => {
    ...
    describe('deleteBook /:id DELETE', () => {
        ...
        it('successfully delete a doc', async () => {
            jest.spyOn(bookService, 'delete').mockImplementationOnce(id => true as any)
    ...
    describe('updateBook /:id PATCH', () => {
        ...
        it('returns the updated value', async () => {
            const result = {} as any
            jest.spyOn(bookService, 'update').mockImplementationOnce((id,update) => result)
        ...
        it('throws  a 404', async () => {
            ...
            expect(async() => controller.updateBook({}as any, generateObjectId(), mockRes))
                .rejects.toThrow(NotFoundException)
        })
    })
