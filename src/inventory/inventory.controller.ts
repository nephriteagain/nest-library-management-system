import { Controller, Get, Post, Param, Res, HttpStatus, Body, Patch } from '@nestjs/common';
...
export class InventoryController {
    ...
    @Get(':id')
    async getInventoryItem(@Param('id') id: ObjectId, @Res() res: Response) : Promise<Response<Document>> {
    ...
    @Patch(':id')
    async updateInventoryItem(@Param('id') id : ObjectId, @Body() changes: Partial<Inventory>, @Res() res: Response) : Promise<Response<Document>> {
        if (isValidObjectId(id)) {
            return res.sendStatus(HttpStatus.BAD_REQUEST)
        }
        const updatedEntry = await this.inventoryService.updateInventoryItem(id, changes)
