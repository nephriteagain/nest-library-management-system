...
describe('AuthController (e2e)', () => {
            ...
            return (
                req(app.getHttpServer())
                    ...
                    .expect((res) => {
                        ...
                        if (!setCookieHeader) {
                            throw new Error(
                                'Expected Set-Cookie header not found',
                            );
                        }
                        ...
                        const httpOnlyCookieExists = setCookieArray.some(
                            (cookie) =>
                                cookie.includes('jwt') &&
                        ...
                        if (!httpOnlyCookieExists) {
                            throw new Error(
                                'Expected HTTP-only cookie not found',
                            );
                        }
                    })
            );
                    ...
                    return (
                        req(app.getHttpServer())
                            ...
                            .expect((res) => {
                                ...
                                const setCookieArray = Array.isArray(
                                    setCookieHeader,
                                )
                                    ? setCookieHeader
                                    : [setCookieHeader];

                                const httpOnlyCookieExists =
                                    setCookieArray.every(
                                        (cookie) =>
                                            !cookie.includes('jwt') &&
                                            !cookie.includes('HttpOnly'),
                                    );
    ...
    describe('/credential POST', () => {
        ...
        it('returns 204 since no jwt cookie found', () => {
            req(app.getHttpServer()).post('/api/auth/credentials').expect(204);
        });
    });
});
