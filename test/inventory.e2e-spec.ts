...
describe('InventoryController (e2e)', () => {
    ...
    beforeAll(async () => {
        const moduleFixture: TestingModule = await Test.createTestingModule({
            imports: [AuthModule, InventoryModule, JwtModule, DatabaseModule],
        }).compile();
    ...
    describe('/ GET', () => {
        ...
        it('returns a 401 when cookie is missing', () => {
            return req(app.getHttpServer()).get('/api/inventory').expect(401);
        });
        ...
        it('returns a 200', () => {
            return (
                req(app.getHttpServer())
                    //NOTE: hard coded id
                    .get(
                        `/api/inventory/find/title?_id=65a10ec8dcabc3c0cfba96d7`,
                    )
                    .set('Cookie', cookie)
                    .expect(200)
                    .expect((res) => {
        ...
        it('returns a 400 when querying a unknown property that DNE', () => {
            return (
                req(app.getHttpServer())
                    //NOTE: hard coded id
                    .get(
                        `/api/inventory/find/unknownproperty?_id=65a10ec8dcabc3c0cfba96d7`,
                    )
                    .set('Cookie', cookie)
                    .expect(400)
            );
